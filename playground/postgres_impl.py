    ...
    async def initdb(self):
        ...
        try:
            ...
            async with self.pool.acquire() as connection:
                # Create and load extensions in proper order
                await connection.execute('CREATE EXTENSION IF NOT EXISTS vector;')
                await connection.execute('CREATE EXTENSION IF NOT EXISTS age;')
                
                graph_name = os.getenv("AGE_GRAPH_NAME", "default")
                ...
                try:
                    ...
                    # Initialize table structure with dummy entity
                    await connection.execute(f"""
                        SELECT * FROM ag_catalog.cypher('{graph_name}', $$
                            CREATE (n:Entity {{node_id: "dummy_1"}}),
                                   (m:Entity {{node_id: "dummy_2"}}),
                                   (n)-[r:DIRECTED]->(m)
                            RETURN n, m, r
                        $$) as (n agtype, m agtype, r agtype)
                    """)
                    
                    # Clean up dummy entities
                    await connection.execute(f"""
                        SELECT * FROM ag_catalog.cypher('{graph_name}', $$
                            MATCH (n:Entity {{node_id: "dummy_1"}})-[r:DIRECTED]->(m:Entity {{node_id: "dummy_2"}})
                            DELETE r, n, m
                            RETURN count(r)
                        $$) as (count agtype)
                    """)
                    
                    logger.info("Initialized Entity and DIRECTED table structures")
                ...
                tables_sql = [
                    ...
                    # Document status table (if not already created)
                    """
                    CREATE TABLE IF NOT EXISTS LIGHTRAG_DOC_STATUS (
                        workspace VARCHAR(255),
                        id VARCHAR(255),
                    ...
                    # Vectors table (if not already created)
                    """
                    CREATE TABLE IF NOT EXISTS LIGHTRAG_VECTORS (
                        workspace VARCHAR(255),
                        id VARCHAR(255),
                ...
                logger.info("PostgreSQL extensions, graph, and tables initialized")
            
        except Exception as e:
            logger.error(f"Failed to initialize PostgreSQL database at {self.host}:{self.port}/{self.database}")
...
class PGKVStorage(BaseKVStorage):
    ...
    async def get_by_mode_and_id(self, mode: str, id: str) -> Union[dict, None]:
        ...
        params = [self.db.workspace, mode, id]
        
        try:
            array_res = await self.db.query(sql, params, multirows=True)
            return array_res[0] if array_res else None
        except Exception as e:
            logger.error(f"Error in get_by_mode_and_id: {e}")
            logger.error(f"SQL: {sql}")
            logger.error(f"Params: {params}")
            return None
    ...
    async def filter_keys(self, keys: set[str]) -> set[str]:
        ...
        try:
            # Convert keys to list and create numbered placeholders
            key_list = list(keys)
            placeholders = ','.join(f'${i+2}' for i in range(len(key_list)))  # Start from $2
            ...
            # Create params dictionary with workspace and keys
            params = {
                "workspace": self.db.workspace,
                **{f"${i+2}": key for i, key in enumerate(key_list)}
            }
            
            result = await self.db.query(sql, params=params, multirows=True)
            
            # If no results found, return all input keys
            if not result:
                return set(keys)
            
            # Extract existing IDs and return difference
            existing_ids = {row["id"] for row in result if row and "id" in row}
            return set(keys) - existing_ids
            
        except Exception as e:
            logger.error(f"Error in filter_keys: {e}")
            logger.debug(f"SQL: {sql}")
            logger.debug(f"Params: {params}")
            logger.debug(f"Input keys: {keys}")
            return set()
...
SQL_TEMPLATES = {
    ...
    "get_by_id_doc_status": """
        SELECT id, content, content_summary, content_length, status, chunks_count, 
    ...
    "get_by_id_llm_cache": """
        SELECT id, original_prompt, COALESCE(return_value, '') as return_value, mode
        FROM LIGHTRAG_LLM_CACHE 
        WHERE workspace = $1 AND id = $2
    """,
    
    "get_by_mode_id_kv_store_llm_response_cache": """
        SELECT id, original_prompt, COALESCE(return_value, '') as return_value, mode
        FROM LIGHTRAG_LLM_CACHE 
